...
lazy_static! {
    pub static ref CRATE_NAME_REGEX: Regex =
        Regex::new(r#"^[a-zA-Z][a-zA-Z0-9-_]{0,63}$"#).unwrap();
}
...
impl actix_web::FromRequest for CargoAuth {
    ...
    fn from_request(req: &HttpRequest, _: &mut actix_web::dev::Payload) -> Self::Future {
        ...
        futures::future::ready(Err(error::ErrorUnauthorized(
            "no authorization header found or invalid. ex. 'Authorization: username:token'",
        )))
    }
}
...
async fn authenticate<'a>(config: &Arc<config::Config>, auth: &'a CargoAuth) -> Result<&'a str> {
    match config
        .user_provider
        .authenticate_user(&auth.user_id, &auth.token)
        .await
    {
        ...
        Err(e) => {
            config
                .log_ingestor
                .error(format!("error authenticating user: {:?}", e));
...
pub(super) async fn new(
    ...
    if !ascii_check(&metadata) {
        return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
            detail: "invalid non-ascii characters present in metadata".to_string(),
        }])));
    }
    let metadata: config::ApiCrate = serde_json::from_str(&metadata)
        .map_err(|e| error::ErrorBadRequest(format!("malformed json: {:?}", e)))?;
    ...
    if !CRATE_NAME_REGEX.is_match(&metadata.name) {
        return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
            detail: "crate name must match the following regex: ^[a-zA-Z][a-zA-Z0-9-_]{0,63}$"
                .to_string(),
        }])));
    }
    if metadata.authors.len() < 1 {
        return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
            detail: "you must specify at least 1 author in your Cargo.toml".to_string(),
        }])));
    }
    if metadata.description.is_none() || metadata.description.as_ref().unwrap().trim() == "" {
        return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
            detail: "you must specify a description in your Cargo.toml".to_string(),
        }])));
    }
    for (feature_name, features) in metadata.features.iter() {
        if !ascii_check(feature_name) || features.iter().any(|x| !ascii_check(x)) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in features".to_string(),
            }])));
        }
    }
    for author in metadata.authors.iter() {
        if !ascii_check(author) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in authors".to_string(),
            }])));
        }
    }
    if let Some(description) = &metadata.description {
        if !ascii_check(description) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in description".to_string(),
            }])));
        }
    }
    if let Some(documentation) = &metadata.documentation {
        if !ascii_check(documentation) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in documentation".to_string(),
            }])));
        }
    }
    if let Some(homepage) = &metadata.homepage {
        if !ascii_check(homepage) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in homepage".to_string(),
            }])));
        }
    }
    if let Some(readme) = &metadata.readme {
        if !ascii_check(readme) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in readme".to_string(),
            }])));
        }
    }
    if let Some(readme_file) = &metadata.readme_file {
        if !ascii_check(readme_file) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in readme_file".to_string(),
            }])));
        }
    }
    for keyword in metadata.keywords.iter() {
        if !CRATE_NAME_REGEX.is_match(keyword) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in keywords".to_string(),
            }])));
        }
    }
    for keyword in metadata.categories.iter() {
        if !CRATE_NAME_REGEX.is_match(keyword) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected categories in categories".to_string(),
            }])));
        }
    }
    if let Some(license) = &metadata.license {
        if !ascii_check(license) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in license".to_string(),
            }])));
        }
    }
    if let Some(license_file) = &metadata.license_file {
        if !ascii_check(license_file) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in license_file".to_string(),
            }])));
        }
    }
    if let Some(repository) = &metadata.repository {
        if !ascii_check(repository) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in repository".to_string(),
            }])));
        }
    }
    for (name, data) in metadata.badges.iter() {
        if !CRATE_NAME_REGEX.is_match(name)
            || data
                .iter()
                .any(|(name, value)| !ascii_check(name) || !ascii_check(value))
        {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in badges".to_string(),
            }])));
        }
    }
    if let Some(links) = &metadata.links {
        if !ascii_check(links) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in links".to_string(),
            }])));
        }
    }
    for dependency in metadata.deps.iter() {
        if !CRATE_NAME_REGEX.is_match(&dependency.name) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in dependency name".to_string(),
            }])));
        }
        if dependency.features.iter().any(|x| !ascii_check(x)) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in dependency features".to_string(),
            }])));
        }
        if let Some(target) = &dependency.target {
            if !ascii_check(target) {
                return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                    detail: "unexpected tokens in dependency target".to_string(),
                }])));
            }
        }
        if !ascii_check(&dependency.kind) {
            return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                detail: "unexpected tokens in dependency kind".to_string(),
            }])));
        }
        if let Some(registry) = &dependency.registry {
            if !ascii_check(registry) {
                return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                    detail: "unexpected tokens in dependency registry".to_string(),
                }])));
            }
        }
        if let Some(explicit_name_in_toml) = &dependency.explicit_name_in_toml {
            if !ascii_check(explicit_name_in_toml) {
                return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
                    detail: "unexpected tokens in dependency explicit_name_in_toml".to_string(),
                }])));
            }
        }
    }
    if existing_crate.is_none()
        && !config
            .crate_provider
            .check_name_availability(&metadata.name)
            .await
            .map_err(|e| {
                config.log_ingestor.error(format!(
                    "error checking name availability for crate crate: {:?}",
                    e
                ));
                error::ErrorServiceUnavailable("service unavailable")
            })?
    {
        return Ok(Json(RegistryResponse::Errors(vec![RegistryError {
            detail: "crate name was taken by a similar crate".to_string(),
        }])));
    }
    ...
    let warnings = config
        ...
        .map_err(|e| {
            config
                .log_ingestor
                .error(format!("error publishing crate: {:?}", e));
            error::ErrorServiceUnavailable("service unavailable")
        })?;
...
pub(super) async fn yank(
    req: HttpRequest,
    auth: CargoAuth,
    info: web::Path<CrateInfo>,
) -> Result<Json<RegistryResponse>> {
    let config: &Arc<config::Config> = req.app_data().unwrap();
    let username = authenticate(config, &auth).await?;
    ...
    config
        .crate_provider
        .yank(&info.crate_name, &info.version)
        .await
        .map_err(|e| {
            config
                .log_ingestor
                .error(format!("error yanking crate: {:?}", e));
            error::ErrorServiceUnavailable("service unavailable")
        })?;
...
pub(super) async fn unyank(
    ...
    config
        .crate_provider
        .unyank(&info.crate_name, &info.version)
        .await
        .map_err(|e| {
            config
                .log_ingestor
...
pub(super) async fn download(req: HttpRequest, info: web::Path<CrateInfo>) -> Result<HttpResponse> {
    ...
    match &ccrate.fetcher {
        config::CrateFetch::DirectFetch(fetcher) => {
            let body = fetcher.fetch(&ccrate).await.map_err(|e| {
                config
                    .log_ingestor
                    .error(format!("error pulling crate: {:?}", e));
                error::ErrorServiceUnavailable("service unavailable")
            })?;
...
pub(super) async fn get_owners(
    req: HttpRequest,
    info: web::Path<PartialCrateInfo>,
) -> Result<Json<OwnerUsersResponse>> {
    let config: &Arc<config::Config> = req.app_data().unwrap();

    let ccrate = get_crate(config, &info.crate_name, None).await?;
    Ok(Json(OwnerUsersResponse::Users(
        ccrate
            .owners
            .into_iter()
            .map(|owner| OwnerUser {
...
pub(super) async fn add_owners(
    req: HttpRequest,
    auth: CargoAuth,
    info: web::Path<PartialCrateInfo>,
    body: web::Json<OwnerChangeRequest>,
) -> Result<Json<OwnerChangeResponse>> {
    let config: &Arc<config::Config> = req.app_data().unwrap();
    let username = authenticate(config, &auth).await?;
    ...
    config
        .crate_provider
        .update_owners(&ccrate.api_crate.name, &ccrate.api_crate.vers, new_owners)
        .await
        .map_err(|e| {
            config
                .log_ingestor
                .error(format!("error updating owners on crate: {:?}", e));
            error::ErrorServiceUnavailable("service unavailable")
        })?;
    Ok(Json(OwnerChangeResponse {
        ok: true,
        msg: format!(
            "users have been added as owners of crate {}",
            ccrate.api_crate.name
        ),
    }))
}

pub(super) async fn remove_owners(
    req: HttpRequest,
    auth: CargoAuth,
    info: web::Path<PartialCrateInfo>,
    body: web::Json<OwnerChangeRequest>,
    ...
    let ccrate = authorize_crate(config, username, &info.crate_name, None).await?;
    let new_owners = ccrate
        .owners
        .clone()
        .into_iter()
        .filter(|x| !body.0.users.contains(x) || x == username)
        .collect();
    ...
    Ok(Json(OwnerChangeResponse {
        ok: true,
        msg: format!(
            "users have been removed as owners of crate {}",
            &ccrate.api_crate.name
        ),
    }))
}
...
pub(super) async fn search(
    ...
    let crates = config
        .crate_provider
        .search(&args.q, per_page)
        .await
        .map_err(|e| {
            config
                .log_ingestor
                .error(format!("error searching for crates: {:?}", e));
            error::ErrorServiceUnavailable("service unavailable")
        })?;
    Ok(Json(SearchResults {
        meta: SearchResultMeta {
            total: crates.len() as u32,
        },
        crates,
    }))
}
