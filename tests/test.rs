...
impl CrateProvider for MockProvider {
    ...
    async fn index(&self) -> Result<Vec<IndexCrate>> {
        ...
        Ok(crates.crates.iter().map(|(_, x)| x.iter().map(|(_, x)| x)).flatten().map(RegistryCrate::make_index).collect())
    }
    ...
    async fn publish(&self, ccrate: RegistryCrate) -> Result<Warnings> {
        ...
        if !crates.crates.contains_key(&ccrate.api_crate.name) {
            crates.crates.insert(ccrate.api_crate.name.clone(), BTreeMap::new());
        }
    ...
    async fn search(&self, query: &str, _count: u32) -> Result<Vec<SearchResult>> {
        ...
        match exact_match {
            None => Ok(vec![]),
            Some(ccrate) => Ok(vec![SearchResult { name: ccrate.api_crate.name, max_version: ccrate.api_crate.vers, description: ccrate.api_crate.description }]),
        }
    }
...
pub async fn host() {
    ...
    let provider = Arc::new(MockProvider {
        crate_store: RwLock::new(CrateStore { crates: BTreeMap::new() }),
    });
    spawn_within_actix(Config::new("0.0.0.0:8080", "./index", "http://127.0.0.1:8080/".parse().unwrap(), provider.clone(), provider, None).unwrap()).await;
}